...
def venta(precio_venta):
	try:
		venta_balance = cliente.get_asset_balance(asset='CHZ')
		print(venta_balance)
	...
	presupuesto=round(float(venta_balance["free"]),1)
	precio_venta=round(precio_venta*1.001,4)
...
def compra(precio_compra,presupuesto):
	...
	if presupuesto>11 and presupuesto*0.1>11:
		...
		cantidad_gasto=12
	precio_bajo=round(precio_compra*0.999,4)
	precio_alto=round(precio_compra*1.001,4)
	cantidad_quantily=round(float(cantidad_gasto/precio_bajo),1)
...
def inicio():
	cliente=Client(usuario1.api_key,usuario1.secret_key)
	presupuesto_balance = cliente.get_asset_balance(asset='USDT')
	print(presupuesto_balance)
	print("Mi presupuesto de USDT: ",presupuesto_balance["free"])
	moneda="CHZUSDT"
...
while conexion==True:
	try:
		...
		if it==0:
			...
			compra_balance = cliente.get_asset_balance(asset="USDT")
			...
			#order=cliente.create_test_order(symbol=moneda,side="BUY",type="LIMIT",timeInForce="GTC",quantity=100,price='100')
		...
		if presupuesto_venta>(10/media_precio):
			print("SE BUSCA LA VENTA")
			ordenes=-1
			
		else:
			if presupuesto_vendiendo>(10/media_precio) or presupuesto_comprando>10:
				ordenes=1
			else:
				ordenes=0	
		...
		if suma_precio>0 and ordenes==0:
			if (condicion_1=="decreciendo" or condicion_2=="libro_ordenes_compra"):
				...
				print("SE DA LA ORDEN DE COMPRA")
